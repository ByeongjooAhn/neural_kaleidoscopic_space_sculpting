...
def get_camera_params_subpixel(uv, pose, intrinsics):
    if pose.shape[1] == 7:  # In case of quaternion vector representation
        cam_loc = pose[:, 4:]
        R = quat_to_rot(pose[:, :4])
        p = torch.eye(4).repeat(pose.shape[0], 1, 1).cuda().float()
    ...
    depth = torch.ones((batch_size, num_samples)).cuda()
    x_cam = uv[:, :, 0].clone().view(batch_size, -1)
    y_cam = uv[:, :, 1].clone().view(batch_size, -1)
    z_cam = depth.view(batch_size, -1)

    # sub-pixel sampling
    x_cam += torch.rand_like(x_cam) - 0.5
    y_cam += torch.rand_like(y_cam) - 0.5
...
def get_camera_params_concat(uv, pose, intrinsics, img_res, pose_all):
    idx = torch.div(uv[:, :, 1], img_res[0], rounding_mode='floor').long().squeeze()
    uv[:, :, 1] = uv[:, :, 1] % img_res[0]
    ...
    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics).squeeze(0)

    # unsqueeze for batch matrix product
    pixel_points_cam = pixel_points_cam.unsqueeze(-1)

    world_coords = torch.bmm(p, pixel_points_cam)[:, :3].squeeze(2)
    ray_dirs = world_coords - cam_loc[:, :]
    ray_dirs = F.normalize(ray_dirs, dim=1)

    return ray_dirs.unsqueeze(0), cam_loc.unsqueeze(0)
...
def get_sphere_intersection(cam_loc, ray_directions, r=1.0):
    ...
    cam_loc = cam_loc.reshape(n_imgs * n_pix, 3, -1)
    ray_directions = ray_directions.reshape(n_imgs * n_pix, -1, 3)
    ray_cam_dot = torch.bmm(ray_directions, cam_loc).squeeze()
    under_sqrt = ray_cam_dot ** 2 - (cam_loc.reshape(-1, 3).norm(2, 1) ** 2 - r ** 2)
...
def get_depth(points, pose):
    ...
    points_hom = torch.cat((points, torch.ones((batch_size, num_samples, 1), device=device)), dim=2)
